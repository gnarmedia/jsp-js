...
describe('class TagLib', () => {
    describe('cleanCodeExp()', () => {
        it('should clear the ${} from a code expression', () => {
            const codeExpText = '${success && otherSuccess || otherValue > 5}';
            const cleanedText = 'success && otherSuccess || otherValue > 5';

            expect(tagLib.cleanCodeExpText(codeExpText)).toEqual(cleanedText);
        });
        it('should return the already clean expression', () => {
            const cleanedText = 'success && otherSuccess || otherValue > 5';

            expect(tagLib.cleanCodeExpText(cleanedText)).toEqual(cleanedText);
        });
    });
